...
"""
Full credit to Tony Duan's diffusion repo for the network architecture.
This file copied from:
https://github.com/tonyduan/diffusion/blob/main/src/blocks.py
"""
...
class PositionalEmbedding(nn.Module):
    ...
    def make_embedding(dim, max_length=10000):
        ...
        div_term = torch.exp(
            torch.arange(0, dim, 2) * (-math.log(max_length / 2 / math.pi) / dim)
        )
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_c, out_c, time_c):
        ...
        self.conv2 = nn.Conv2d(
            out_c, out_c, kernel_size=3, stride=1, padding=1, bias=False
        )
    ...
    def forward(self, x, t):
        out = self.conv1(x)
        out = self.bn1(out)
        temb = self.mlp_time(t)
        temb = repeat(temb, "b -> 1 b 1 1")
        out = F.relu(out + temb)
        out = self.conv2(out)
        out = self.bn2(out)
...
class SelfAttention2d(nn.Module):
    ...
    def forward(self, x):
        ...
        o = rearrange(
            o, "(b g) c (h w) -> b (g c) h w", g=self.num_heads, w=x.shape[-1]
        )
...
class UNet(nn.Module):
    ...
    def forward(self, x, t):
        ...
        for block in self.up_blocks:
            if isinstance(block, BasicBlock):
                if skip_conns[-1].shape[-1] != x.shape[-1]:
                    diff = x.shape[-1] - skip_conns[-1].shape[-1]
                    skip_conns[-1] = F.pad(skip_conns[-1], (0, diff, 0, diff))
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        self, dim_q, dim_k, dim_v, num_heads=8, dropout_prob=0.1, dim_a=None, dim_o=None
    ):
        super().__init__()
        if dim_a is None:
            dim_a = dim_q
...
class ViT(nn.Module):
    ...
    def __init__(self, in_dim, embed_dim, num_layers, patch_shape):
        ...
        self.patch_embed = nn.Linear(
            in_dim * math.prod(patch_shape), embed_dim, bias=True
        )
        ...
        self.out_embed = nn.Linear(
            embed_dim, in_dim * math.prod(patch_shape), bias=True
        )
    ...
    def forward(self, x, t):
        ...
        x = self.patch_embed(x) + rearrange(
            self.pos_encoding[: x.shape[1]], "t c -> () t c"
        )
        t = self.time_embed[t]
